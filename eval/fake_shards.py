        ...
        with open(path.join(args.outdir,'-'.join(map(str,ratio))+'.qrel'), 'w') as wf:
            ...
            for qnum, docs in sorted(query_docs.items()):
                for i, doc in enumerate(docs):
                    ...
                    curr_ratio = ref_rank*100//1000
                    if curr_ratio > 100:
                        # if we're beyond rank 100, generate a random shard of the shard
                        # candidate list to add this document to
                        curr_ratio = randint(0, 100)
                    else:
